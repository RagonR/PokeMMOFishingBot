...
import pyautogui, time, PIL, threading, subprocess, os
from tkinter import *
...
class botInterface(Frame):
    def __init__(self):
        ...
        # Info
        self.labKill = Label(frame, text="List of pokemons: Catch/Kill (BOT MADE FOR 'Good Rod')", bg="#87DEF2", font=font11)
        ...
        # Checkbox
        self.Horsea = IntVar()
        self.Horbut = Checkbutton(frame, bg="#87DEF2", text="Horsea    ", variable=self.Horsea, font=font11, onvalue=1, offvalue=0)
        self.Horbut.grid(row=6, column=0, sticky=W+E)
        self.Magikarp = IntVar()
        self.MagBut = Checkbutton(frame, bg="#87DEF2", text="Magikarp", variable=self.Magikarp, font=font11, onvalue=1, offvalue=0)
        self.MagBut.grid(row=7, column=0, sticky=W+E)
        self.Krabby = IntVar()
        self.KarBut = Checkbutton(frame, bg="#87DEF2", text="Karbby    ", variable=self.Krabby, font=font11, onvalue=1, offvalue=0)
...
def setup():
    ...
    messagebox.showwarning("Worning", "Make sure that 'False Swipe' is first attack and to kill one second (best to use 20PP+)")
# Fishing
def checking(p1):
...
def loadsetting():
    with open("save.txt", "r") as load:
        Pokemones.Pokeballs, Pokemones.TillHeal, Pokemones.Rod, Pokemones.catchHor, Pokemones.catchMag, Pokemones.catchKra = [str(x) for x in next(load).split()]
    load.close()
